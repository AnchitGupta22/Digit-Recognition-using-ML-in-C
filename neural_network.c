...
void neural_network_softmax(float * activations, int length)
{
    ...
    // checks for the maximum activation value
    ...
    for (i = 0, sum = 0; i < length; i++) {
        // normalising activation value to prevet large exponents
...
float neural_network_gradient_update(mnist_image_t * image, neural_network_t * network, neural_network_gradient_t * gradient, uint8_t label)
{
    ...
    for (i = 0; i < MNIST_LABELS; i++) {
        ...
        // since correctly activated neuron will have activation = 1,
        // gradient for it will be 0
        // else it will correctly tune the bias weight to achieve correct activation
...
float neural_network_training_step(mnist_dataset_t * dataset, neural_network_t * network, float learning_rate)
{
    ...
    // memset is a function used to fill a block of memory with specific value
